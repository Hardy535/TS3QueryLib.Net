...
namespace TS3QueryLib.Core
{
    public class SyncFileTransfer : FileTransferBase
    {
        ...
        public static void UploadFile(string fileTransferKey, string host, ushort filePort, ulong numberOfBytesToSkip,
            ulong numberOfBytesToSend, string sourceFileName)
        {
        ...
        public static void UploadData(string fileTransferKey, string host, ushort filePort, ulong numberOfBytesToSkip,
            ulong numberOfBytesToSend, Stream sourceStream)
        {
        ...
        {
            Socket socket =
                new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp) {ReceiveBufferSize = 4096};
        ...
        protected static void ReadFromStreamWriteToSocket(SocketAsyncEventArgs socketAsyncEventArgs,
            ulong numberOfBytesToSend, Stream sourceStream)
        {
            ...
            do
            {
                ...
                {
                    resultError = args.SocketError;
                    bytesTransfered = args.BytesTransferred;
                    sendLock.Set();
                };
            ...
            if (bytesSent < numberOfBytesToSend)
                throw new InvalidOperationException(string.Format(
                    "File upload failed and was aborted after sending {0} of {1} bytes.", bytesSent,
                    numberOfBytesToSend));
        }

        protected static void ReadFromSocketWriteToStream(SocketAsyncEventArgs socketAsyncEventArgs, ulong size,
            Stream downloadStream)
        {
            SocketAsyncEventArgsUserToken userToken = (SocketAsyncEventArgsUserToken) socketAsyncEventArgs.UserToken;
            ulong bytesRead = 0;
            ...
            do
            {
                ...
                EventHandler<SocketAsyncEventArgs> receiveCallback = (sender, args) =>
                {
                    ...
                    if (bytesTransfered != 0)
                        downloadStream.Write(socketAsyncEventArgs.Buffer, socketAsyncEventArgs.Offset,
                            socketAsyncEventArgs.BytesTransferred);

                    receiveLock.Set();
                };

                userToken.Socket.InvokeAsyncMethod(userToken.Socket.ReceiveAsync, receiveCallback,
            ...
            if (bytesRead < size)
                throw new InvalidOperationException(string.Format(
                    "File download failed and was aborted after reading {0} of {1} bytes.", bytesRead, size));
        }
    }
}